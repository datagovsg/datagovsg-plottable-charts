    ...
    if (this.plot.labelsEnabled) {
      ...
      new Plottable.Interactions.Pointer()
        .onPointerMove(point => {
          const target = this.plot.entitiesAt(point)[0]
          if (target) {
            const idx = this.props.labels.indexOf(target.dataset.metadata())
            const textLabel = this.plot.content()
              .selectAll('.bar-label-text-area').filter((d, i) => i === idx)
              .selectAll('.bar-label-text-area > g').filter((d, i) => i === target.index)
            ...
            tooltipAnchorSelection.attr({
              ...
              'data-original-title': 'Value: ' + this.props.labelFormatter()(target.datum.value)
            })
