...
var PivotTable = function () {
  function PivotTable(data) {
    classCallCheck(this, PivotTable);

    this.data = data;
    this.transformations = [];
  }

  createClass(PivotTable, [{
    key: 'push',
    value: function push() {
      var _transformations;

      (_transformations = this.transformations).push.apply(_transformations, arguments);
    }
  }, {
    key: 'transform',
...
function filterItems(field, filter) {
  var filterFunc = function (_field, _filter) {
    ...
    if ((typeof _filter === 'undefined' ? 'undefined' : _typeof(_filter)) === 'object') {
      if (_filter.type === 'include' && _filter.values instanceof Array) {
        return function (d) {
          return _filter.values.indexOf(get_1(d, _field)) > -1;
        };
      }
      if (_filter.type === 'exclude' && _filter.values instanceof Array) {
        return function (d) {
          return _filter.values.indexOf(get_1(d, _field)) < 0;
        };
      }
    }
    throw new TypeError();
  ...
  return function (data) {
    return data.map(function (g) {
      return Object.assign({}, g, { _items: g._items.filter(filterFunc) });
    });
  };
}

function filterGroups(field, filter) {
  var filterFunc = function (_field, _filter) {
    if (typeof _field === 'function') return _field;
    if (typeof _field !== 'string') throw new TypeError();
    if ((typeof _filter === 'undefined' ? 'undefined' : _typeof(_filter)) === 'object') {
      if (_filter.type === 'include' && _filter.values instanceof Array) {
        return function (g) {
          return _filter.values.indexOf(get_1(g, _field)) > -1;
        };
      }
      if (_filter.type === 'exclude' && _filter.values instanceof Array) {
        return function (g) {
          return _filter.values.indexOf(get_1(g, _field)) < 0;
        };
      }
    }
    throw new TypeError();
  ...
  return function (data) {
    ...
    data.forEach(function (g) {
      ...
      g._items.forEach(function (d) {
        var groupValue = get_1(d, field);
        labels[groupValue] = groupValue;
      });
      var grouped = groupBy_1(g._items, function (d) {
        return get_1(d, field);
      });
      Object.keys(grouped).forEach(function (groupKey) {
        var items = grouped[groupKey];
        if (items.length === 0) return;
        result.push({
          _group: Object.assign({}, g._group, defineProperty$2({}, field, labels[groupKey])),
          _items: items,
          _summaries: []
        });
      });
...
function aggregate(labelField, valueField) {
  ...
  if (typeof aggregateFunc === 'function') {
    return function (data) {
      return data.map(function (g) {
        var labels = {};
        g._items.forEach(function (d) {
          var groupValue = get_1(d, labelField);
          labels[groupValue] = groupValue;
        });
        var grouped = groupBy_1(g._items, function (d) {
          return get_1(d, labelField);
        });
        var series = [];
        var decimalPlaces = 0;
        Object.keys(grouped).forEach(function (groupKey) {
          ...
          if (values.length > 0) {
            ...
            series.push({ label: label, value: aggregateFunc(values) });
          }
        });
        ...
        g._summaries.push({ type: type, labelField: labelField, valueField: valueField, decimalPlaces: decimalPlaces, series: series });
        return g;
      });
    };
  }
