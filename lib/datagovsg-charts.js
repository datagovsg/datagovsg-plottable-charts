...
function getCategoryScale() {
  var _DATAGOVSG_CONFIG$get = DATAGOVSG_CONFIG.getCategoryScale,
      innerPadding = _DATAGOVSG_CONFIG$get.innerPadding,
      outerPadding = _DATAGOVSG_CONFIG$get.outerPadding;

  return new Plottable.Scales.Category().innerPadding(innerPadding).outerPadding(outerPadding);
}
...
function getCustomTickGenerator() {
  var targetLineCount = DATAGOVSG_CONFIG.getCustomTickGenerator.targetLineCount;
  ...
  return function (scale) {
    ...
    var nearestPower = Math.pow(10, Math.floor(Math.log(domainRange / targetLineCount) / Math.LN10));
    var factor = domainRange / targetLineCount / nearestPower;
    factor = [10, 5, 2, 1, 0.5].filter(function (v) {
      return v <= factor;
    })[0];
    var interval = nearestPower * factor;
...
var helpers = Object.freeze({
	DATAGOVSG_CONFIG: DATAGOVSG_CONFIG,
 ...
 * }
 */
function highlightOnHover(component) {
  if (component.plot.markers) {
    new Plottable.Interactions.Pointer().onPointerMove(function (point) {
      component.plot.markers.entities().forEach(function (e) {
        e.selection.classed('highlight', false).style('stroke', '').style('fill', '');
      });
      var target = component.plot.markers.entityNearest(point);
      if (target) {
        target.selection.classed('highlight', true).style('stroke', 'black').style('fill', 'white');
      }
    }).onPointerExit(function (point) {
      component.plot.markers.entities().forEach(function (e) {
        e.selection.classed('highlight', false).style('stroke', '').style('fill', '');
